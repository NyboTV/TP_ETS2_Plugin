const gaugeStates = async (TPClient, telemetry_path, logIt, timeout, images_path) => {
    // Loading Module
    const fs = require('fs')
    ...
    async function loop () {
        ...
        if(fs.readFileSync(`${path}/config/usercfg.json`).gaugeStates === false) {
		...
		let image_arrow = await Jimp.read(`${images_path}/images/Gauge.png`);
		let image2_Speed = await Jimp.read(`${images_path}/images/SpeedGauge.png`);
		let image2_RPM = await Jimp.read(`${images_path}/images/RPMGauge.png`);
		...
		let Fuel = Math.round(telemetry.fuel)
		let FuelCapacity = Math.round(telemetry.fuelCapacity)
		
		// Module Stuff
		if (Location === "mph") {
			Speed = Math.round(telemetry.speed / 2 + (telemetry.speed / 100 * 10))
		} else {
